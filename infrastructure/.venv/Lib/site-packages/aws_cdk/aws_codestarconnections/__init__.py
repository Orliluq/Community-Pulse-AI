...
class CfnConnection(
    ...
    *Note:* A connection created through CloudFormation is in ``PENDING`` status by default. You can make its status ``AVAILABLE`` by updating the connection in the console.
    ...
    def attr_connection_arn(self) -> builtins.str:
        ...
        The ARN is used as the connection reference when the connection is shared between AWS services. For example: ``arn:aws:codestar-connections:us-east-2:123456789012:connection/39e4c34d-e13a-4e94-a886-ea67651bf042`` .
